...
const TipoDocEdit = ({ tipoDoc, onVolver }) => {
    ...
    const confirmarGuardado = async () => {
        ...
        try {
            setGuardando(true);
            await axios.put(`/api/tipos-doc/${tipoDoc.id}`, formData);
            setExito('Tipo de documento actualizado correctamente');
            setTimeout(() => onVolver(), 1500);
        } catch (error) {
            setApiError(error.response?.data?.error || 'Error al actualizar el tipo de documento');
        } finally {
            setGuardando(false);
        }
    };
        ...
        <>
            ...
            <div className="d-flex justify-content-between align-items-start mb-4">
                <div>
                    <h5 className="fw-bold mb-1">
                        Editar Tipo de Documento: <span style={{ color: 'var(--primary)' }}>{tipoDoc.nombre}</span>
                    </h5>
                    <p className="text-muted small mb-0">Modifica el nombre o descripción del tipo de documento</p>
                </div>
            ...
            <div className="card border">
                <div className="card-header bg-light d-flex align-items-center gap-3 py-3">
                    <i className="bi bi-file-earmark-text-fill" style={{ fontSize: 22, color: 'var(--primary)' }} />
                    <div>
                        <p className="fw-semibold mb-0 small">Datos del Tipo de Documento</p>
                        <p className="text-muted mb-0" style={{ fontSize: 12 }}>
                            Este tipo está disponible al crear expedientes en el sistema.
                        </p>
                    </div>
                </div>
                    ...
                    <div className="border-bottom">
                        <div className="px-4 py-2 bg-light border-bottom">
                            <span className="small text-muted fw-bold text-uppercase" style={{ letterSpacing: '0.06em' }}>
                                <i className="bi bi-info-circle me-1" />Datos del Tipo de Documento
                            </span>
                        </div>
