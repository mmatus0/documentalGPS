...
const FilaIntegrante = ({ usuario, esYo }) => {
  ...
  return (
    <tr className={esYo ? 'table-active' : ''}>
      <td className="ps-4 py-3">
        <div className="d-flex align-items-center gap-2">
          ...
          <div>
            ...
            {esYo && <div style={{ fontSize: 10, color: 'var(--primary)', fontWeight: 600 }}>— Tú</div>}
          </div>
        </div>
      </td>
      ...
      <td className="py-3">
        <span className={`badge ${badgeGlobal.bg}`} style={{ fontSize: 10 }}>{usuario.rol_global}</span>
      </td>
    </tr>
  );
};
...
const MiUnidadDetalle = ({ unidad, usuario, onVolver, onVerExpedientes }) => {
  ...
  const areaId  = unidad.area_id ?? unidad.id;

  const fetchIntegrantes = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const { data } = await axios.get(`/api/areas/${areaId}/usuarios`);
      setIntegrantes(data);
    } catch (err) {
  ...
  useEffect(() => { fetchIntegrantes(); }, [fetchIntegrantes]);
  ...
  const listaTab = tabActiva === 'todos' ? integrantes
    : tabActiva === 'colaboradores' ? colaboradores
    : lectores;
    ...
    <div>
      <div className="d-flex gap-2 mb-4">
        <button className="btn btn-sm btn-outline-secondary" onClick={onVolver}>
          <i className="bi bi-arrow-left me-1" />Volver a mis unidades
        </button>
        <button className="btn btn-sm btn-primary" onClick={() => onVerExpedientes(unidad)}>
          <i className="bi bi-folder2-open me-1" />Ver expedientes
        </button>
      </div>
      ...
      <div className="card border-0 shadow-sm mb-4" style={{ borderRadius: 12, overflow: 'hidden' }}>
        ...
        <div className="card-body px-4 py-4">
          <div className="row align-items-center g-3">
            ...
            <div className="col-md-4 text-md-end">
              <div className="d-inline-flex align-items-center gap-2 px-3 py-2 rounded-pill"
                ...
                <div>
                  <div style={{ fontSize: 11, fontWeight: 600, color: permiso.color }}>{permiso.titulo}</div>
          ...
          <div className="mt-3 d-flex align-items-start gap-2 px-3 py-2 rounded"
            style={{ background: '#f8fafc', border: '1px solid var(--border)' }}>
            <i className="bi bi-info-circle text-muted mt-1" style={{ fontSize: 13, flexShrink: 0 }} />
            <p className="mb-0 text-muted" style={{ fontSize: 12, lineHeight: 1.5 }}>{permiso.descripcion}</p>
          </div>
        </div>
      </div>
      ...
      {!loading && !error && (
        <div className="row g-3 mb-4">
          {[
            { label: 'Total integrantes', valor: integrantes.length, icon: 'bi-people-fill', color: 'var(--primary)', bg: 'var(--primary-light)' },
            { label: 'Colaboradores', valor: colaboradores.length, icon: 'bi-pencil-fill', color: '#10b981', bg: '#d1fae5' },
            { label: 'Lectores', valor: lectores.length, icon: 'bi-eye-fill', color: '#64748b', bg: '#f1f5f9' },
          ].map(stat => (
        ...
        <div className="card-body p-0">
          {loading ? (
            <div className="text-center py-5 text-muted">
              <div className="spinner-border spinner-border-sm me-2" role="status" />Cargando integrantes…
            </div>
          ) : error ? (
            <div className="p-4">
              <div className="alert alert-danger d-flex align-items-center gap-2 mb-0">
                ...
                <span>{error}</span>
                <button className="btn btn-sm btn-outline-danger ms-auto" onClick={fetchIntegrantes}>Reintentar</button>
              </div>
            </div>
          ) : (
            <div className="table-responsive">
              <table className="table table-hover align-middle mb-0">
                ...
                <tbody>
                  {listaTab.length === 0 ? (
                    <tr><td colSpan="4" className="text-center text-muted py-5 small">No hay integrantes en esta categoría</td></tr>
                  ) : (
                    listaTab.map(u => (
                      <FilaIntegrante key={u.id} usuario={u} esYo={u.correo === usuario.correo} />
                    ))
                  )}
                </tbody>
              </table>
