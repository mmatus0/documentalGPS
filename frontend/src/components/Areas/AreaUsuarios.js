...
const AreaUsuarios = ({ areaId: areaIdProp, areaNombre, onVolver }) => {
    const modoEmbebido = Boolean(areaIdProp);

    const [areas,            setAreas]            = useState([]);
    const [areaSeleccionada, setAreaSeleccionada] = useState(modoEmbebido ? String(areaIdProp) : '');
    ...
    useEffect(() => {
        if (!modoEmbebido) {
            axios.get('/api/areas')
                .then(r => setAreas(r.data.filter(a => a.estado_id === 1)))
                .catch(() => setAreas([]));
        }
    }, [modoEmbebido]);
                ...
                <table className="table table-hover mb-0">
                    <thead className="table-light">
                        <tr>
                            ...
                            <th className="small text-muted fw-semibold text-end pe-4">Acción</th>
                        </tr>
                    </thead>
                    <tbody>
                        {lista.length === 0 ? (
                            ...
                            lista.map(u => (
                                <tr key={u.id}>
                                    <td className="ps-4 fw-medium small">{u.nombre_completo}</td>
    ...
    return (
        <>
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4 className="fw-bold mb-1">
                        {modoEmbebido
                            ? `Usuarios del área: ${areaNombre}`
                            : 'Asignación de Usuarios a Unidades Organizativas'}
                    </h4>
                    <p className="text-muted small mb-0">
                        Administra qué usuarios pertenecen a cada Área como Colaborador o Lector
                    </p>
                </div>
                {modoEmbebido && onVolver && (
                    <button className="btn btn-outline-secondary btn-sm" onClick={onVolver}>
                        <i className="bi bi-arrow-left me-1" />Volver a Áreas
                    </button>
                )}
            </div>
            ...
            {areaSeleccionada && (
                <>
                    {feedback && (
                        <div className={`alert alert-${feedback.type === 'ok' ? 'success' : 'danger'} alert-dismissible d-flex align-items-center`} role="alert">
                            <span>{feedback.msg}</span>
                            <button type="button" className="btn-close ms-auto" onClick={() => setFeedback(null)} />
                        </div>
                    )}
                                ...
                                <div className="row g-3 align-items-end">
                                    <div className="col-md-5">
                                        ...
                                        {disponibles.length === 0 && !errors.usuario_id && (
                                            <div className="form-text">Todos los usuarios activos ya están asignados a esta área</div>
                                        )}
                                    </div>
                                    <div className="col-md-5">
                    ...
                    {loadingGrupos ? (
                        ...
                        <div className="row g-4">
                            <div className="col-md-6">
                                <TablaGrupo titulo={<><i className="bi bi-pencil-fill me-2" />Colaboradores</>} lista={colaboradores} badgeVariant="success" />
                            </div>
                            <div className="col-md-6">
                                <TablaGrupo titulo={<><i className="bi bi-eye-fill me-2" />Lectores</>} lista={lectores} badgeVariant="secondary" />
                            </div>
                        </div>
                    )}
                </>
