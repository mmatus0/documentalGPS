...
const TipoColabList = ({ onNuevo, onEditar, onVolver }) => {
    ...
    const fetchTipos = useCallback(async () => {
        try {
            const { data } = await axios.get('/api/tipos-colab');
            setTipos(data);
        } catch {
            console.error('Error al cargar tipos de colaboración');
        }
    }, []);
    ...
    const handleDesactivar = (id, nombre) => {
        setModal({
            visible: true,
            titulo:  'Desactivar Tipo de Colaboración',
            mensaje: `¿Desea desactivar "${nombre}"? Las tareas que lo usan no se verán afectadas.`,
            labelConfirmar: 'Desactivar',
            variante: 'danger',
            onConfirmar: async () => {
                await axios.delete(`/api/tipos-colab/${id}`);
                cerrarModal();
                fetchTipos();
            },
        });
    ...
    const handleReactivar = (id, nombre) => {
        setModal({
            visible: true,
            titulo:  'Reactivar Tipo de Colaboración',
            mensaje: `¿Desea reactivar "${nombre}"?`,
            labelConfirmar: 'Reactivar',
            variante: 'primary',
            onConfirmar: async () => {
                await axios.patch(`/api/tipos-colab/${id}/reactivar`);
                cerrarModal();
                fetchTipos();
            },
        });
        ...
        <>
            <div className="d-flex justify-content-between align-items-start mb-4">
                <div>
                    ...
                    <h5 className="fw-bold mb-1">Tipos de Colaboración</h5>
                    <p className="text-muted small mb-0">
                        Define los tipos de colaboración disponibles al solicitar apoyo dentro de una tarea activa.
                    </p>
                </div>
            ...
            <div className="card border">
                <div className="d-flex justify-content-between align-items-center px-4 border-bottom">
                    ...
                    <input
                        ...
                        placeholder="Buscar tipo de colaboración..."
                        style={{ maxWidth: 240 }}
                        value={busqueda}
                            ...
                            {listaFiltrada.length === 0 ? (
                                <tr>
                                    <td colSpan="3" className="text-center text-muted py-5 small">
                                        {busqueda
                                            ? 'No se encontraron tipos de colaboración con ese criterio'
                                            : `No hay tipos de colaboración ${tabActiva}`}
                                    </td>
                                </tr>
                            ) : (
                                listaFiltrada.map(t => (
